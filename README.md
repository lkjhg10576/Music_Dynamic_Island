...
- **置于任务栏 (NEW)** — 将灵动岛锁定至屏幕左下角任务栏区域，开启后禁止拖拽移动，自动吸附到任务栏上方，支持一键切换锁定/解锁状态
...
│       ├── screenshot4.jpg       # 通知效果截图
│       ├── screenshot5.png       # 硬件监控截图
│       ├── screenshot6.png       # 硬件监控截图
│       ├── qq.png                # QQ 应用图标
│       ├── dingtalk.png          # 钉钉应用图标
│       ├── wechat.png            # 微信应用图标
│       ├── mail.png              # 邮件应用图标
...
│   ├── src/
│   │   ├── main.rs               # Rust 入口
│   │   └── lib.rs                # 核心逻辑：Tauri 命令、托盘、窗口管理、Windows DWM 样式、网易云音乐信息捕获、媒体控制、多源封面获取、系统通知监听（含 AUMID 获取）、系统硬件监控数据采集、消息点击唤醒应用、静默置顶（焦点避让）
...
- `control-hardware-mon` — 控制台 → 灵动岛硬件监控开关同步
- `control-pin-taskbar` — 控制台 → 灵动岛置于任务栏开关同步
...
| `fetch_latest_notification` | 通过 Windows `UserNotificationListener` 读取系统通知中心最新 Toast 通知，返回应用名、标题、内容和 AUMID（自动过滤微信） |
| `open_app_by_aumid` | 通过应用 AUMID 或协议 URI 打开对应程序，支持 QQ、微信、钉钉等常用应用的协议唤醒，内置焦点防劫持处理 |
| `force_window_topmost` | 静默置顶灵动岛窗口（不抢占焦点），自动检测并避让右键菜单 |
...
1. **启用条件**：音乐控制器开启时默认启用流光边框，关闭时同步关闭；首次启用音乐控制器时自动开启并持久化到 localStorage
2. **状态持久化 (NEW)**：流光边框开关状态保存至 localStorage（`nsd_glow_border`），重启应用后自动恢复上次设置
3. **实现原理**：
...
4. **透明度联动**：边框可见度 = 悬浮窗不透明度的 gamma 校正值（`alpha = pow(linear, 1/2.2)`），确保视觉协调
5. **用户控制**：右键菜单提供「开启/关闭流光边框」选项，始终可用（不再受音乐控制器状态限制）
...
4. **内容解析**：提取 Toast 通知的 Visual Binding 文本元素，通过 `AppInfo.DisplayName` 获取精准程序名称，`text[0]` 为标题，`text[1..]` 拼接为正文内容，同时获取应用的 AUMID（AppUserModelId）用于后续唤醒
5. **智能过滤**：自动识别并忽略微信（WeChat）相关通知，避免频繁的消息弹窗干扰
6. **应用图标映射**：根据应用名称自动匹配对应图标（QQ、钉钉、微信、邮件等），未匹配时使用默认 Logo
7. **点击唤醒应用 (NEW)**：点击通知区域自动调用 `open_app_by_aumid` 打开对应的应用程序：
   - **协议唤醒**：QQ 使用 `tencent://message/`，微信使用 `weixin://`，钉钉使用 `dingtalk://`
   - **UWP 唤醒**：通过 `shell:AppsFolder\{AUMID}` 方式启动 UWP 应用
   - **焦点防劫持**：底层模拟按一下 Alt 键释放系统焦点锁定，确保应用能正常前台显示
8. **灵动岛展示**：收到新通知时：
   - 触发 `animateIslandSize(360, 65)` 将灵动岛弹性扩展至通知尺寸
   - 显示应用图标头像 + 通知标题（14px 加粗）+ 通知正文（12.5px）
   - 5 秒后自动收回至原始胶囊尺寸（260x42）
9. **优先级**：通知 > 音乐 > 网速（即通知到来时优先展示通知内容）
...
将灵动岛固定在任务栏上方，提供持续可见的监控体验：

1. **开启流程**：用户在「灵动岛设置面板」中开启「置于任务栏」开关 -> 状态写入 localStorage（`nsd_pin_taskbar`）-> 通过 `control-pin-taskbar` 事件推送至 WidgetIsland
2. **位置吸附**：调用 `snapToBottomLeft()` 自动定位到屏幕左下角，紧贴任务栏上方（Y坐标 = 屏幕底部 - 窗口高度 - 3px微调）
3. **防遮挡策略**：使用"隐身移动法"——先隐藏窗口，设置位置后再显示，绕过 Windows 系统的任务栏防遮挡机制
4. **拖拽锁定**：开启置于任务栏后，自动拦截所有鼠标拖拽操作，右键菜单中「重置位置」按钮变为禁用状态
5. **通知尺寸限制**：置于任务栏状态下，收到系统通知时灵动岛不会扩展尺寸，避免遮挡任务栏内容
6. **解锁恢复**：关闭置于任务栏开关后，自动调用 `adjustWindowPosition()` 恢复到屏幕顶部居中位置
...
4. 灵动岛支持：
   - **左键拖拽** — 自由移动位置（置于任务栏状态下禁用）
   - **右键菜单** — 重置位置 / 开启或关闭流光边框 / 关闭悬浮窗
   - **置于任务栏** — 在设置面板中开启后自动吸附到屏幕左下角，锁定位置禁止拖拽
...
6. 点击顶部 **「灵动岛设置」按钮** 进入灵动岛专属设置面板：
   ...
   - **消息通知接收 (PRO)** — 开启后灵动岛将实时展示系统通知：
     ...
     - 5 秒后自动收回原始尺寸
     - 自动过滤微信通知避免干扰
     - **点击唤醒** — 点击通知区域自动打开对应应用（支持 QQ、微信、钉钉等协议唤醒）
   ...
   - **置于任务栏 (NEW)** — 将灵动岛锁定至屏幕左下角任务栏区域：
     - 开启后自动吸附到任务栏上方，禁止拖拽移动
     - 右键菜单中「重置位置」按钮变为禁用状态
     - 收到通知时灵动岛不会扩展尺寸，避免遮挡任务栏
     - 关闭后自动恢复到屏幕顶部居中位置
