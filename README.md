...
这是一个 Windows 桌面灵动岛组件。屏幕顶部的小岛会实时显示网速与正在播放的音乐，并支持流量统计、系统通知、系统状态提醒、实时活动（专注番茄钟 / 倒计时 / 硬件监控 / 健康提醒 / 系统动态感知 / 打印机队列监控）与硬件监控；多个实时活动可同时并行，也能吸附在任务栏左下角。
...
- **多源封面获取**：优先使用系统本地高清封面，自动降级到网络封面，再不行用渐变图兜底
- **封面缓存**：智能缓存最近 50 首歌曲封面，提升响应速度
- **彩虹流光边框**：8 色渐变旋转边框，可独立开关
- **音频频谱可视化**：实时显示随音乐节奏跳动的 5 段频谱
- **歌名滚动**：长歌名自动水平滚动，展开后切换为双行显示
- **智能交互**：悬停显示控件，离开后自动切回歌曲信息，1 秒自动收缩
- **弹性按压展开**：点击音乐岛触发弹簧动画展开，显示完整控制按钮
- **音乐进度条**：展开态显示播放进度（已播/总时长/剩余），支持拖动定位
...
> 在灵动岛上以通知形式提醒你系统状态的变化。

- **四类提醒**：音量变化 / 电源插拔 / 电量提醒 / 解锁提示
- **灵动岛通知**：相关事件会在灵动岛弹出提示，看完自动收起，不占用常驻空间
- **逐项开关**：设置页可分别开启或关闭每一类提醒；只要有一类开着，总开关即为开启
- **音量智能合并**：连续调节音量时不会刷屏，只在结束时提示最终音量；过长的文字也会自动省略避免遮挡
- **网络断连提醒**同样由本功能的总开关统一管理
...
- **久坐提醒**：可设间隔（默认 1 小时），到点灵动岛弹出“该起来走走了”，并播放系统提示音，支持「跳过一次 / 忽略」
- **喝水提醒**：可设间隔（默认 2 小时），到点灵动岛弹出“该喝水了”，同样带系统提示音，支持「跳过一次 / 忽略」
- **实时同步**：灵动岛与实时活动面板同步各提醒的剩余时间 / 提醒中状态；提醒期间会持续提示直到你处理
...
- **智能避让**：收到系统通知或健康提醒独占时，小图标会自动隐藏，避免遮挡
...
- **实时活动面板**：画廊式设置界面，统一管理六大功能模块与优先级
- **省内存模式**：开启后关闭主窗口彻底释放内存，托盘点击时重建窗口
...
| 媒体控制 | Windows SMTC |
| 系统接口 | Windows 系统 API |
| 本地存储 | Rust 侧 config.json（单一数据源）+ app_data_dir 落盘 |
...
│   ├── views/
│   │   ├── MainPanel.vue         # 主控制台（设置、统计、音乐、实时状态、下拉导航）
│   │   ├── WidgetIsland.vue      # 灵动岛悬浮窗（状态编排，逻辑拆分至 composables）
│   │   └── LiveActive.vue        # 实时活动枢纽
│   ├── components/
│   │   ├── SpeedChart.vue        # 网速 / CPU / 内存趋势图
│   │   ├── StatsChart.vue        # 本地流量统计图表
│   │   ├── DynamicSet.vue        # 灵动岛设置
│   │   ├── PersonalizeCenter.vue # 个性化中心
│   │   └── LyricsManager.vue     # 歌词管理（缓存列表 + 单首配置）
│   ├── composables/              # 灵动岛按职责拆分的组合式函数
│   │   ├── useMusicSync.ts       # 音乐同步（SMTC 事件驱动 + 封面缓存）
│   │   ├── useLyrics.ts          # 歌词（LRC 解析 + setTimeout 精确调度）
│   │   ├── useIslandAnimation.ts # 尺寸动画 / 宽度调整
│   │   ├── useRealtimeActivity.ts# 实时活动（番茄钟/倒计时/健康/硬件/轮换）
...
│   │   ├── music_controller.rs   # 音乐控制器（SMTC / 歌词搜索 / 曲目聚合）
│   │   ├── session_binder.rs     # SMTC 会话绑定管理器（事件驱动音乐推送）
│   │   ├── lyrics_cache.rs       # 歌词缓存（唯一 key + 落盘 + 索引）
...
│   │   ├── traffic_stats.rs      # 流量统计（按天累计 + 落盘）
│   │   └── config_store.rs       # 设置单一数据源（config.json + 广播）
│   ├── Cargo.toml                # Rust 依赖
...
7. 点击“LiveActive 设置”进入实时活动枢纽，配置六大模块（专注番茄钟 / 快捷倒计时 / 硬件监控 / 健康提醒 / 系统动态感知 / 打印机队列监控）与优先级
...
本项目（Music Dynamic Island / MDI）基于 **NetSpeed Dynamic Pro（NSD Pro）** 衍生开发：原项目由 Ryen (GEORGEWU) 以 MIT 协议开源，其 MIT 许可正文已保留在 [LICENSE.md](./LICENSE.md) 前部；本衍生版本（含全部新增与修改）采用 Apache License 2.0 重新授权，版权归 lkjhg10576 所有。完整许可证文本见 [LICENSE.md](./LICENSE.md)。
